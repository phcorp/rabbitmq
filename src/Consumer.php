...
class Consumer {
   ...
   *   Is is valid?
   */
  public function isQueueNameValid($queueName) {
  ...
  public function decode($data) {
    $decoder = isset($this->decoder) && NULL !== $this->decoder ? $this->decoder : 'json_decode';
    return $decoder($data);
  }
   ...
   */
  public function consumeQueueApi($queueName) {
    $this->preFlightCheck();
    $this->startListening();
    $worker = $this->getWorker($queueName);
    // Allow obtaining a decoder from the worker to have a sane default, while
   ...
   *   Did it ?
   */
  protected function hitIterationsLimit($maxIterations, $iteration) {
   ...
   */
  protected function hitMemoryLimit($memoryLimit) {
    // Evaluate whether worker should exit.
    // If the --memory_limit option is set, check the memory usage
    // and exit if the limit has been exceeded or met.
    if ($memoryLimit > 0) {
   ...
   *   its items.
   */
  public function shutdownQueue($queueName) {
    $queue = $this->queueFactory->get($queueName);
